...
| `/privacy`, `/terms`, `/support`     | Server Component | Public legal/support pages (no auth — not in `middleware.ts` PROTECTED_ROUTES; listed in `PUBLIC_ROUTES`). `/support` (App Store Connect Support URL): what Chefer is, in-app Send feedback + `SUPPORT_EMAIL` (`@chefer/types`), FAQ (password reset, delete account, AI use, Premium pricing). `/privacy` is the full GDPR Art. 13 notice and `/terms` the full Terms of Service (backlog P0-6, both "Effective <date>" at the top, `EFFECTIVE_DATE` constant; `/privacy#analytics` is linked from the Profile analytics card) — see §15 "Privacy & analytics consent" |
...
- **`previewVideo(user, url)`** (2026-09-26, owner decision "Gemini must go") — a video link becomes a **draft for the user to review**, not a Cheferize diff. Same `RECIPE_IMPORT` reservation (premium-only; refunded on any failure). `VideoRecipeService.extract` (below) reads the video's words; the result is sanitized and returned as `{ via: 'video', draft, notFound, unverifiedQuantities, assumptions, transcriptSource, safety, platform, sourceUrl, ogImageUrl, videoTitle, creator }`. `notFound` (`name`/`ingredients`/`instructions`/`servings`/`time`) and `unverifiedQuantities` are decided **deterministically from the source text** (`lib/video-import/text.ts`), never from the model's own claims — it fills servings and times even when nobody said them. `safety` runs the P1-2 matcher over the draft against the household's allergies/restrictions as a warning; no Cheferize call. The client saves the reviewed draft through `save` as the `original` variant. `ogImageUrl` is the YouTube thumbnail only (TikTok/Instagram CDN links are signed and expire).
...
### VideoRecipeService (application layer) — video link → recipe, from its words

`apps/api/src/application/video-import/video-recipe.service.ts`. One pipeline for the in-app import (`recipe.importVideoPreview` → the review form) and the offline CURATED-dataset tool (`pnpm recipes:from-video`). Extraction only — it writes nothing.

**Words only, no Gemini video (owner decision 2026-09-26).** No model ever receives video frames or audio. The transcriber (`lib/video-import/`) turns the link into text, and that text goes through the ordinary text extraction (`IAIService.extractRecipeAnnotated({ text })`, the `importText` route, so any provider — `AI_ROUTE_IMPORT_TEXT`). The annotated prompt's _no invented method_ rule lets a gap come back empty (→ "not found — please add" in the form) instead of as plausible invented steps.

**Transcript pipeline** (`lib/video-import/transcript.ts`, `VideoTranscriber`), cheapest first, stopping at the first source that carries a recipe:

1. **Caption/description** — `yt-dlp --dump-single-json --skip-download` (metadata only). If the caption alone carries a whole recipe (≥3 ingredient-looking lines AND ≥2 method sentences, `captionCarriesRecipe`), nothing else is fetched.
2. **Subtitles / auto-captions** — one track: uploaded subtitles in the video's language (else English, else any), otherwise the ORIGINAL auto-caption track (`*-orig`), never one of YouTube's ~100 machine translations. VTT/SRT flattened, YouTube's rolling duplicate lines removed (`subtitlesToText`). A failure here just moves on.
3. **Speech** — audio only: `yt-dlp -f bestaudio/best` (TikTok/Instagram serve muxed clips; the video track is stripped) → `ffmpeg` to 16 kHz mono 32 kbps mp3 (~0.24 MB/min; Whisper resamples to 16 kHz anyway) → Groq Whisper (`GroqWhisperClient`: `POST {AI_SECONDARY_BASE_URL}/audio/transcriptions`, `model=WHISPER_MODEL`, `response_format=verbose_json`; checked against console.groq.com/docs/speech-to-text 2026-09-26). Segments with `no_speech_prob ≥ 0.6` and the classic "thanks for watching" hallucination are dropped. Skipped when `AI_SECONDARY_API_KEY` is unset.

A caption that carries only part of the recipe (an ingredient list with no method) rides along with steps 2–3, and is used on its own when they come up empty — the form then asks for the rest.

**Caps, timeouts, clean-up, errors.**

- URL allowlist first (`@chefer/utils` `parseVideoUrl`: single YouTube/Shorts, TikTok and Instagram reel/post links only — no channels, playlists or other hosts), and yt-dlp is additionally restricted with `--ies youtube,YoutubeYtBe,TikTok,vm.tiktok,Instagram`, so a URL can never reach its generic extractor (no SSRF surface). The URL always follows `--`.
- `VIDEO_MAX_SECONDS` (default 600) refused from metadata before any download; `--match-filter "!is_live & duration <=? N"` plus ffmpeg `-t N` cover videos with no reported duration. `VIDEO_MAX_DOWNLOAD_MB` (default 50) via `--max-filesize`; the transcoded file must fit Groq's 25 MB.
- Per-step timeouts: probe 45 s, subtitles 45 s, audio download 120 s, ffmpeg 90 s, Whisper 90 s.
- Every temp dir (`chefer-subs-*`, `chefer-audio-*`) is removed in a `finally`. Nothing is stored: audio is deleted right after transcription, and only the text reaches the model.
- `VideoImportError` codes → friendly sentences (`errors.ts`): `UNSUPPORTED_SITE`, `PRIVATE` (login/age wall), `NOT_FOUND`, `BLOCKED` (YouTube's datacenter-IP bot check, 429s), `TOO_LONG`, `TOO_LARGE`, `NO_SPEECH`, `TIMEOUT`, `UNAVAILABLE` (binary missing, Whisper 429/5xx), `FAILED`. The service maps them to `BAD_REQUEST` / `TIMEOUT` / `SERVICE_UNAVAILABLE` TRPCErrors carrying only the friendly copy; raw yt-dlp stderr goes to the log.
- **Mock mode:** `AI_MOCK_ENABLED=true` swaps in `MockVideoTranscriber` (no yt-dlp, no Whisper), steered by the URL: `private`, `too-long`, `no-speech` → errors; `caption-only` → ingredient list with no method; `subtitles` → subtitles source; `satay`/`beef`/`no-recipe` → the mock extractor's fixtures.

**Serving-count trust** (`lib/video-import/servings.ts`) — `servings` is the least reliable field and the most damaging when wrong (one clip yielded 2, 3, 5, 5 and 2 across five runs, each reported "high"). When the words state no explicit yield (`serves 4`, `4 servings` — _not_ "per serving"), confidence is capped at `medium` and the form flags servings as "not stated".

**Batch driver:** `scripts/extract-video-recipes.ts` (`pnpm recipes:from-video <url…>` / `--file urls.txt --out drafts.json`) — reports how many drafts came from caption / subtitles / speech. Output carries `stage`, `confidence`, `assumptions`, `sourceText`, `sourceUrl`, `creator`; drafts go to review before promotion to CURATED.

**Legal note.** Downloading from TikTok and Instagram (and YouTube) may conflict with those platforms' terms of service. Chefer only fetches the single link a user explicitly submits (no crawling, no batch fetching from the app), reads the words rather than the footage, stores no media (audio is deleted right after transcription), and saves the result to the user's private collection with the source link. The offline dataset tool is a manual, paced batch (`--delay`). Review this with counsel before promoting video-derived recipes to the shared CURATED pool.
...
- AI out of capacity (429/quota, or the friendly SERVICE_UNAVAILABLE): the
  remaining batches retry after 90 s, doubling per consecutive failure up to
  1 h (`capacityRetryDelayMs`), so an exhausted free daily quota is not
  hammered; a clean pass resets the back-off.
...
- `previewTargets(userId, metrics)` → `MacroTargets & { lifter }` — backs
  ...
  `lifter: { bodyweightKg, proteinGPerKg }` set) when `loadLifter` says the user
  is a lifter. The Adaptive Chef dial is not applied.
...
| `openai.ts`            | `OpenAICompatibleAIService` — generic client for any OpenAI-compatible `/chat/completions` endpoint (`AI_SECONDARY_BASE_URL`/`AI_SECONDARY_MODEL`/`AI_SECONDARY_API_KEY`; default Groq free tier, `openai/gpt-oss-120b`). JSON mode + shape instruction, same Zod gates, same shared prompts. **Vision** via `AI_VISION_MODEL` (default `qwen/qwen3.8-27b`): photo calls send an `image_url` part with a base64 data URL; HEIC is flagged failover-worthy. **Video never** (Gemini-only). **Chunked meal plan** when it leads the meal-plan route: 7 sequential per-day calls with strict `json_schema` (falls back to `json_object` if the endpoint rejects it, or for one call when strict generation fails), dishes already planned passed forward, assembled week validated with `weekPlanResponseSchema`; days paced against the `x-ratelimit-*` token budget (rate-limit.ts). Text calls send `AI_SECONDARY_REASONING_EFFORT`. One repair retry on invalid JSON/Zod output. Background calls retry one ≤10 s 429. Also serves **Cloudflare Workers AI** (`cloudflare` provider: `https://api.cloudflare.com/client/v4/accounts/{id}/ai/v1`, always chunked plans, `neuronBudget` gate → 429 capacity error once the day's text share is spent, `usage.neurons` logged) and an optional `fastModel` for `FAST_MODEL_OPS` (prices, shopping, review) |
| `rate-limit.ts`        | Groq rate-limit helpers: Retry-After / "try again in Xs" parsing and `TokenRateState` (TPM refill estimate from `x-ratelimit-*` headers) for chunk pacing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `cloudflare-budget.ts` | Workers AI neuron accounting (free-only mode): `NeuronLedger` (per UTC day, in memory, shared by the text client and the image service), `CF_NEURON_RATES` (pricing page, 2026-09-26; fallback only — every Workers AI response reports `usage.neurons`), `textBudgetAllows` (`CF_TEXT_NEURON_BUDGET`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `routing.ts`           | Workloads (`mealPlan`, `swap`, `cheferize`, `importText`, `vision`, `chat`, `review`, `prices`, `shopping`), `DEFAULT_AI_ROUTES` (= the pre-2026-09 table), `AI_ROUTE_*` / `AI_SHADOW_ROUTE` parsing, `resolveRoutes` (drops providers without keys)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `failover.ts`          | `ChainAIService` — each call runs down its workload's ordered provider chain; capacity/quota errors (`isCapacityAiError`), HTTP 413 and `failover: true` errors move to the next provider, anything else throws. Hands each served call to the shadow hook. `FailoverAIService` = the chain with the default routes (Gemini + one secondary). Logs which provider served each call                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `providers.ts`         | Env-free provider construction (`createProvider`), shared by the factory and the eval harness                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `env-schema.ts`        | The AI provider env shape (keys, model names, defaults), spread into `lib/env.ts` and parsed alone by the eval harness                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `call-context.ts`      | Per-request AI call context (AsyncLocalStorage): `{ userId, premium, shadow? }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `shadow.ts`            | `ShadowRunner` — samples premium, consented, user-initiated calls (`AI_SHADOW_ROUTE`, `AI_SHADOW_SAMPLE`), replays them on the candidate chain after the response (setImmediate, max 2 in flight, never throws), discards the output and logs `[ai.shadow]` scores. Chat is never shadowed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `consent.ts`           | `hasAiDataConsent(userId)` — fail-closed TODO hook until PR #45's `users.aiDataConsentAt` lands (answers "no", so shadow mode is inert)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `eval/`                | `pnpm ai:eval` harness: golden-set loader, 429-aware runner, pure scorer (gate: no errors, 0 allergen violations, schema ≥ `--min-schema`), CLI. See `docs/ai-providers.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `usage.ts`             | `logAiUsage` — one `[ai.usage]` JSON log line per model call (provider, model, op, input/output tokens, ms; `shadow: true` on shadow replays) from both live providers and the Cloudflare image service; `onAiUsage` listener (eval token counts); `logAiShadow` writes `[ai.shadow]` lines. Measures real per-feature token volume before the provider swap (P0-5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
**Switching providers:** `AI_PROVIDER=gemini` (+ `GEMINI_API_KEY`) makes `gemini` available; `AI_SECONDARY_API_KEY` makes `groq` (the OpenAI-compatible secondary) available. Each workload then follows its `AI_ROUTE_*` chain (unset = today's table: plan/swap/cheferize/text import/review `gemini>groq`; chat/prices/shopping `groq>gemini`; vision `gemini`). `AI_PROVIDER=openai` runs the secondary alone for every workload (photos use `AI_VISION_MODEL`, plans are chunked, `GEMINI_API_KEY` is not required); the exact Groq-only production lines are in `docs/ai-providers.md` "Running without Gemini". The runbook for moving a workload off Gemini (eval → a week of shadow → flip) is `docs/ai-providers.md`. No code changes required.
...
| `profile.aiProviders`               | Public    | Query    | — → `{ primary, backups }` (`AiProviderDisclosure`, ids `gemini`\|`groq`\|`cloudflare`): the AI providers that receive user data, derived from the live route table (`lib/ai/index.ts` `aiProviderDisclosure`). Drives the consent sheet, the Profile "AI & your data" toggle and the web privacy page / support FAQ, so the copy follows `AI_FREE_ONLY`. Clients fall back to `DEFAULT_AI_PROVIDER_DISCLOSURE` until it answers                                                                                                                         |
| `profile.getAiUsage`                | Protected | Query    | — → today's per-type counts, `geminiTotal` (all AI calls, name kept for shipped clients), `limits`, and (additive 2026-09-26) `primaryProvider { id, name, requestsPerDay, requestsPerMinute }` for the admin telemetry card                                                                                                                                                                                                                                                                                                                             |
...
| `preferences.computeTargets`        | Protected | Query    | `{ goal, biologicalSex, age, heightCm, weightKg, activityLevel }` → `{ dailyCalorieTarget, proteinG, carbsG, fatG, *Pct, lifter }` — the preferences macro preview (web + mobile). Lifters (set-up gym + bodyweight) get the g/kg protein rule like the dashboard; `lifter: { bodyweightKg, proteinGPerKg } \| null` is additive                                                                                                                                                                                                                         |
...
| `recipe.importVideoPreview`         | Protected | Mutation | `{ url }` (YouTube / Shorts / TikTok / Instagram single-video link, `parseVideoUrl`) — reads the video's caption → subtitles → Whisper speech transcript into an editable DRAFT via the text extraction (no Gemini video); same `recipeImportsPerDay` quota (premium-only: FREE → FORBIDDEN), refunded on failure; returns `{ via: 'video', draft, notFound, unverifiedQuantities, assumptions, transcriptSource, safety, platform, sourceUrl, ogImageUrl, videoTitle, creator }`. The reviewed draft is saved with `importSave` (`variant: 'original'`) |
...
| `tracker.getDay`                    | Protected | Query    | `{ date: YYYY-MM-DD }` — planned meals + the day's log (incl. custom entries) + resolved targets; planned meals carry `portion?` (P1-1, macros stay per serving) and `slotIndex` (index in the plan day's `meals`; clients key rows by it). GAIN_MUSCLE lifters also get optional `trainingDay` and (premium, training day) `adjustedTargets` — same as `dashboard.summary` (P2-4 follow-up); `targets` stays the base                                                                                                                                   |
| `tracker.upsertDay`                 | Protected | Mutation | `{ date, loggedMeals[] (0–50, bounded macros) }` — entries are `recipeId` XOR `custom { name, estimatedBy }`, with an optional `slotIndex` (the plan slot a planned entry was ticked from, so two identical snacks tick separately); MERGES: replaces the planned-recipe entries the client manages, keeps custom and off-plan entries (echoed custom entries are ignored); returns `{ log, rebalance }` (F4 — rebalance non-null only when future meals were swapped)                                                                                   |
| `tracker.logRecipe`                 | Protected | Mutation | `{ date, recipeId, mealType, portionMultiplier?, slotIndex? }` — cook-mode "Made it!" / Today's "I ate this": atomic, idempotent append of one recipe (with `slotIndex` only that slot's entry is replaced; without it, the entry for the same recipe + meal type) (macros from the stored recipe; recipe must be visible to the caller); returns `{ log, rebalance }`                                                                                                                                                                                   |
...
| `gym.profile.recommend`             | Protected | Query    | `{ days, experience, equipmentAccess }` → template key, reason, alternatives, preview, volume, hints (pure engine, no DB; every preview slot is within the equipment access set)                                                                                                                                                                                                                                                                                                                                                                         |
...
```
publicProcedure     → timingMiddleware   (signed-in: also sets the AI call context, see below)
...
behaviour: only AI shadow mode reads it, to tell a premium user's own request from a free user's
or a background job (workers never set it; `IngredientPriceWorker.wake()` detaches from the
waking request).
...
- **AI data consent (App Store 5.1.2(i))** — `User.aiDataConsentAt`; both clients gate every AI action through a consent guard (web `features/ai-consent/AiConsentProvider.tsx` in the dashboard layout; mobile `src/features/ai-consent/ai-consent-provider.tsx` in the root layout + an `AiConsentHost` nested in each Sheet that can start an AI action, because iOS can't present a Modal from a controller already presenting one). Copy and when-to-ask logic are shared: `AI_CONSENT_COPY` / `AI_CONSENT_FEATURE_DATA` (`@chefer/types`), `needsAiDataConsent` / `aiConsentRequiredFor` (`@chefer/utils`; free curated generation/swaps never ask). User-initiated procedures are not re-checked server-side. Server-initiated jobs filter on it: `WeeklyPlanWorker` only AI-generates for premium users with `aiDataConsentAt` set (query-level filter + logged skip count), and `CoachService.runReviewSweep` writes the template text (no AI) for premium users without consent (`aiSkipped` returned + logged). `RecipeImageWorker` (AI recipe names only) and `IngredientPriceWorker` (global vocabulary) carry no personal data.
- **Uploaded photo files on deletion (backlog P0-6)** — after the `deleteAccount()` transaction commits, `lib/uploads/uploaded-files.ts` `deleteUploadedFiles()` removes the `/uploads/<uuid>.<ext>` files referenced by the account's avatar (`User.image`), own MANUAL recipes and private custom ingredients (`IngredientPrice.imageUrl`). Best effort: a disk error is logged and never undoes the committed delete. `UPLOADS_DIR` now lives in that module (the uploads router re-exports it).
...
| `AI_FREE_ONLY`             | No       | false                                                           | Free-only mode (owner decision 2026-09-26): every workload defaults to `groq>cloudflare`, Gemini is never built and `GEMINI_API_KEY` is not required; startup fails if any `AI_ROUTE_*`/`AI_SHADOW_ROUTE` names `gemini`, or `AI_SECONDARY_API_KEY` is missing. Consent/privacy copy follows (`profile.aiProviders`). See `docs/ai-providers.md` "Free-only mode" |
...
| `AI_SECONDARY_MODEL`       | No       | openai/gpt-oss-120b                                             | Text model id at the secondary endpoint (no vision; photos use `AI_VISION_MODEL`)                                                                                                                                                                                                                                                                                 |
| `AI_VISION_MODEL`          | No       | qwen/qwen3.8-27b                                                | Vision model at the secondary endpoint, used only when `AI_ROUTE_VISION` sends photos there (Groq vision docs, checked 2026-09-26)                                                                                                                                                                                                                                |
| `AI_SECONDARY_FAST_MODEL`  | No       | —                                                               | Optional smaller model at the secondary for the simple JSON workloads (`estimateIngredientPrices`, `generateShoppingList`, `generateReviewText`), e.g. `openai/gpt-oss-20b`. Unset = `AI_SECONDARY_MODEL` for everything                                                                                                                                          |
| `AI_ROUTE_MEAL_PLAN`       | No       | gemini>groq (free-only: groq>cloudflare)                        | Provider chain for the week plan (`gemini`, `groq`, `cloudflare`, joined by `>`). When `groq` leads, plans are generated in 7 per-day chunks; `cloudflare` always chunks                                                                                                                                                                                          |
| `AI_ROUTE_SWAP`            | No       | gemini>groq                                                     | Chain for meal swaps                                                                                                                                                                                                                                                                                                                                              |
| `AI_ROUTE_CHEFERIZE`       | No       | gemini>groq                                                     | Chain for Cheferize (adapting an imported recipe)                                                                                                                                                                                                                                                                                                                 |
| `AI_ROUTE_IMPORT_TEXT`     | No       | gemini>groq                                                     | Chain for recipe extraction from page or pasted text, and from a video link's transcript                                                                                                                                                                                                                                                                          |
| `AI_ROUTE_VISION`          | No       | gemini                                                          | Chain for meal-photo scans and photo recipe import                                                                                                                                                                                                                                                                                                                |
| `AI_ROUTE_CHAT`            | No       | groq>gemini                                                     | Chain for the AI chef chat                                                                                                                                                                                                                                                                                                                                        |
| `AI_ROUTE_REVIEW`          | No       | gemini>groq                                                     | Chain for the weekly coach review prose                                                                                                                                                                                                                                                                                                                           |
| `AI_ROUTE_PRICES`          | No       | groq>gemini                                                     | Chain for ingredient price/macro estimates                                                                                                                                                                                                                                                                                                                        |
| `AI_ROUTE_SHOPPING`        | No       | groq>gemini                                                     | Chain for AI shopping-list consolidation                                                                                                                                                                                                                                                                                                                          |
| `AI_SHADOW_ROUTE`          | No       | —                                                               | Shadow mode: `<workload>:<chain>[,…]`, e.g. `mealPlan:groq`. Any workload but `chat`                                                                                                                                                                                                                                                                              |
| `AI_SHADOW_SAMPLE`         | No       | 0                                                               | Share (0–1) of premium, consented, user-initiated calls replayed on the shadow chain. 0 = off                                                                                                                                                                                                                                                                     |
| `WHISPER_MODEL`            | No       | whisper-large-v3-turbo                                          | Speech-to-text model for video-link import, at `AI_SECONDARY_BASE_URL/audio/transcriptions` with `AI_SECONDARY_API_KEY` (no key = the speech step is skipped). Groq STT docs, checked 2026-09-26                                                                                                                                                                  |
...
| `CF_ACCOUNT_ID`            | No       | —                                                               | Cloudflare account id (Workers AI), required when `IMAGE_PROVIDER=cloudflare`; also the `cloudflare` text provider (free-only mode, or a route naming it)                                                                                                                                                                                                         |
...
| `CF_TEXT_MODEL`            | No       | @cf/openai/gpt-oss-120b                                         | Workers AI text model for the `cloudflare` provider, at `https://api.cloudflare.com/client/v4/accounts/{CF_ACCOUNT_ID}/ai/v1/chat/completions`. Used only when `AI_FREE_ONLY=true` or a route names `cloudflare`                                                                                                                                                  |
| `CF_VISION_MODEL`          | No       | @cf/google/gemma-4-26b-a4b-it                                   | Workers AI photo model (Apache 2.0; Llama 3.2/4 Vision are not licensed to EU-based developers). Sent with `chat_template_kwargs.enable_thinking=false`                                                                                                                                                                                                           |
| `CF_FAST_MODEL`            | No       | —                                                               | Optional cheaper Workers AI model for prices/shopping/review, e.g. `@cf/openai/gpt-oss-20b`                                                                                                                                                                                                                                                                       |
| `CF_TEXT_NEURON_BUDGET`    | No       | 8000                                                            | Text stops going to Workers AI once today's neurons (text + images, per UTC day, in memory; a restart resets it) reach this; images keep the rest of the free 10,000/day. 0–10000                                                                                                                                                                                 |
...
`AI_SECONDARY_REASONING_EFFORT` (optional, default `auto`): `reasoning_effort` sent with the secondary's text-model calls — `auto` = `low` for gpt-oss models and not sent for others, `none`, `low`, `medium` or `high`. Never sent to the vision model. At gpt-oss's default ("medium") effort one meal-plan day spent ~5.6k reasoning tokens and ran out of output budget; see `docs/ai-providers.md` "Groq limits".
...
### External binaries (video-link recipe import)

`lib/video-import/ytdlp.ts` shells out to two binaries, both **baked into `Dockerfile.api`** (runner stage) and pinned:

| Binary   | Version (pin)                                                          | Used for                                                                  |
| -------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `yt-dlp` | `YTDLP_VERSION=2026.08.19`, official musl standalone, SHA-256 per arch | metadata + caption, one subtitle track, audio-only download (speech step) |
| `ffmpeg` | Alpine 3.20 package `ffmpeg~6.1`                                       | strip video, 16 kHz mono 32 kbps mp3 for Whisper                          |

The yt-dlp binary bundles its YouTube JS-challenge solver and is run with `--js-runtimes node` (node is already in the image). Locally: `brew install yt-dlp ffmpeg`. Without them the import answers "Video import isn't available right now" (`UNAVAILABLE`); nothing else in the API depends on them.

**Bumping yt-dlp** — it breaks whenever YouTube/TikTok/Instagram change their sites (symptom: `FAILED`/`NOT_FOUND`/`BLOCKED` spikes in the `[video-import]` log lines for links that open fine in a browser). Check monthly, and immediately on such a spike:

1. Take the new tag from https://github.com/yt-dlp/yt-dlp/releases and the `yt-dlp_musllinux` / `yt-dlp_musllinux_aarch64` lines of its `SHA2-256SUMS`.
2. Set `YTDLP_VERSION`, `YTDLP_SHA256_AMD64`, `YTDLP_SHA256_ARM64` in `infrastructure/docker/Dockerfile.api` (the build fails on a checksum mismatch).
3. `docker build -f infrastructure/docker/Dockerfile.api -t chefer-api-video .` then `docker run --rm chefer-api-video yt-dlp --version` and `docker run --rm chefer-api-video ffmpeg -version`.
4. Smoke one public YouTube cooking video locally (captions path, no Groq), then ship through the normal deploy.

ffmpeg follows the Alpine base image; bump it with the base.
...
- **Privacy & analytics consent (backlog P0-6, 2026-09-26)** — see below
- **TODO:** Replace SHA-256 password hashing (seed only) with bcrypt or argon2
- **TODO:** Implement full JWT access/refresh token flow

### Privacy & analytics consent (backlog P0-6)

**Analytics (web only — `apps/web/src/lib/analytics.ts`).** PostHog EU Cloud (`https://eu.i.posthog.com`), production only.

| Who                                  | What PostHog gets                                                   | Device storage                                   |
| ------------------------------------ | ------------------------------------------------------------------- | ------------------------------------------------ |
| Signed-out visitor                   | Anonymous events; random in-memory ID per page load                 | None (`persistence: 'memory'`)                   |
| Signed in, no consent (the default)  | Same anonymous events — `identify()` is never called                | None                                             |
| Signed in, opted in (Profile toggle) | `identify(userId, { planTier })` — ID + tier only, never name/email | Only our own `chefer.analytics-consent:<userId>` |
| Browser sends Do Not Track           | Nothing (`respect_dnt: true`)                                       | None                                             |

- Session replay, surveys and feature flags are disabled (`disable_session_recording`, `disable_surveys`, `advanced_disable_flags`): unused, and each would touch browser storage. `analytics.test.ts` runs the real SDK in jsdom and asserts that no cookie/localStorage/sessionStorage key is written except the consent key.
- **Decision:** no cookie banner. With in-memory persistence nothing is stored on or read from the device for analytics (ePrivacy Art. 5(3)); anonymous counts rely on legitimate interest. Linking events to an account is opt-in consent (default OFF), because events such as `weight_logged`/`workout_finished` tied to a person can be health data (GDPR Art. 9). The choice is per account **per browser** (localStorage, strictly necessary to remember it) — no schema/API change. Toggle: `features/profile/components/AnalyticsConsentCard.tsx` on `/profile` ("Usage analytics"); withdrawing calls `posthog.reset()` at once. Trade-off: a full page reload starts a new anonymous ID, so cross-reload funnels only work for opted-in users.
- **Mobile:** sends no analytics (no PostHog or Sentry SDK in `apps/mobile/package.json`), so there is nothing to mirror; `mobile_parity_backlog.md` records that any future mobile analytics must follow the same default-off rule.

**Processors named on `/privacy` (evidence).** Keep this table and the page in sync.

| Processor / recipient                                                                    | Evidence                                                                                                                                                                                                 |
| ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Oracle Cloud Infrastructure (Frankfurt)                                                  | §12 "Oracle Always Free ARM" VM; region eu-frankfurt-1 (owner)                                                                                                                                           |
| Google Gemini (standard routing only)                                                    | `apps/api/src/lib/ai/gemini.ts` (`GeminiAIService`), `AI_PROVIDER=gemini` in `.env.production.example`; never built when `AI_FREE_ONLY=true`                                                             |
| Groq (backup AI; primary in free-only mode; video transcription)                         | `apps/api/src/lib/env.ts` `AI_SECONDARY_BASE_URL` default `https://api.groq.com/openai/v1`; `lib/ai/failover.ts`                                                                                         |
| Cloudflare Workers AI (free-only backup; recipe images when `IMAGE_PROVIDER=cloudflare`) | `lib/ai/providers.ts` `cloudflareBaseUrl`, `lib/image-gen/cloudflare.ts`. The page lists the AI recipients from `profile.aiProviders` (`AI_PROVIDERS` in `@chefer/types`), so it follows the live config |
| Sentry (EU region, `ingest.de.sentry.io`)                                                | web `apps/web/src/instrumentation.ts`, `instrumentation-client.ts`; API `apps/api/src/instrument.ts`. Not on mobile                                                                                      |
| PostHog (EU Cloud)                                                                       | `apps/web/src/lib/analytics.ts`                                                                                                                                                                          |
| Email provider (named generically)                                                       | `apps/api/src/lib/email/index.ts` (Resend today; provider not final)                                                                                                                                     |
| Expo (EAS Update)                                                                        | `expo-updates` in `apps/mobile/package.json`; `.github/workflows/deploy.yml` "Publish EAS update" — IP/device only                                                                                       |
| Image generation (Pollinations; Cloudflare optional)                                     | `apps/api/src/lib/image-gen/index.ts` — recipe name + cuisine only, no personal data                                                                                                                     |
| Content hosts loaded by the client (Unsplash, Pollinations, YouTube nocookie)            | `apps/web/src/lib/recipe-image.ts`, `features/gym/library/VideoEmbed.tsx`, mobile `exercise-video-sheet.tsx` — IP only                                                                                   |

Not listed (no user data): DuckDNS / Let's Encrypt (domain + TLS), Carrefour price lookups (ingredient names only), Cloudinary (only with `IMAGE_PROVIDER=cloudflare`, unset in prod).

**Retention stated on `/privacy`:** account data until deletion; VM keeps 14 nightly dumps (`infrastructure/scripts/backup-db.sh`) and the dev-Mac mirror keeps 30 (`pull-backups.sh` `KEEP=30`) → "about 30 days"; sessions 30 days (`auth.service.ts` `SESSION_EXPIRY_MS`); reset links 1 hour (`password-reset.service.ts`). Server-log, Sentry and PostHog retention are stated generically — no explicit periods are configured yet.
