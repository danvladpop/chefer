...
| `eval/`             | `pnpm ai:eval` harness: golden-set loader (`apps/api/eval/golden`), runner, pure scorer (schema %, allergen violations — the gate, restriction violations, kcal/macro error, latency, tokens), CLI. See `docs/ai-providers.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
**Switching providers:** `AI_PROVIDER=gemini` (+ `GEMINI_API_KEY`) makes `gemini` available; `AI_SECONDARY_API_KEY` makes `groq` (the OpenAI-compatible secondary) available. Each workload then follows its `AI_ROUTE_*` chain (unset = today's table: plan/swap/cheferize/text import/review `gemini>groq`; chat/prices/shopping `groq>gemini`; vision `gemini`). `AI_PROVIDER=openai` runs the secondary alone for every workload (smoke-testing; photos use `AI_VISION_MODEL`, plans are chunked). The runbook for moving a workload off Gemini (eval → a week of shadow → flip) is `docs/ai-providers.md`. No code changes required.
