...
- `loadLifter(userId, chefProfile)` → `{ lifterBodyweightKg }` — non-null only
  for a set-up gym profile + goal GAIN_MUSCLE + known bodyweight (latest weight
  log, else the profile). Non-GAIN_MUSCLE users skip the reads. Passed to
  `resolveDailyTargets(profile, lifterBodyweightKg)` (protein 1.8 g/kg) by the
  dashboard, tracker, chat context and both meal-plan generation paths.
