...
| `failover.ts`       | `ChainAIService` — each call runs down its workload's ordered provider chain; capacity/quota errors (`isCapacityAiError`), HTTP 413 and `failover: true` errors move to the next provider, anything else throws. Video extraction is pinned to Gemini. Hands each served call to the shadow hook. `FailoverAIService` = the chain with the default routes (Gemini + one secondary). Logs which provider served each call                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `AI_VISION_MODEL`          | No       | qwen/qwen3.8-27b                                                | Vision model at the secondary endpoint, used only when `AI_ROUTE_VISION` sends photos there (Groq vision docs, checked 2026-09-26). Video is always Gemini                            |
...
| `AI_ROUTE_IMPORT_TEXT`     | No       | gemini>groq                                                     | Chain for recipe extraction from page or pasted text                                                                                                                                  |
