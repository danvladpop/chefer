...
| E2E prod sweep             | ✅ 2026-08-22 — full user-journey test on prod (fresh account, free + premium arcs, data audits, image sweep, mobile 375px, auth flows). Core flows all healthy: 0 safety violations across 40 recipes, macro arithmetic ≤15% error, targets/TDEE math exact, chat answers match plan data, cook→tracker→progress consistent, pin/personalisation/budget honoured, AI consolidation clean, no mobile overflow anywhere. 6 data/UX issues + 4 polish items logged as `docs/prod-followups.md` #3–#9 (headliners: price estimator chokes on units like "g, dry" and "pinch"; tracker macro targets ≠ dashboard targets; 1 dead curated image; generic repeated ingredient thumbnails; 40% of list items categorised "Other"; gain-muscle protein target 3.5 g/kg unrealistic)                                                                                                                   |
| prod-followups fix session | ✅ 2026-08-23 — findings #3–#9 all fixed, deployed and prod-verified (7 commits + a repository follow-up): price-estimator unit parsing (lentils €52.50→€0.35, saffron €22.50→€0.90, curated week €120→€51); tracker targets unified through `resolveDailyTargets` (tracker=dashboard, verified 165/394/83); protein capped at 2.2 g/kg with kcal redistributed to carbs (261g→165g); dead curated image → Pollinations + `upsertRecipes` now treats CURATED fixture images as authoritative; ingredient thumbnails per-ingredient generated (117 poisoned cache rows purged, 0 shared images on a 101-item list); category map +70 keywords + -es/-ies plural matcher bug ("Other" 41%→18%); Chefer branding on auth/landing; "Generate My Week" auto-generates via `?generate=1`. Remaining open: #2 (low-confidence chat-send, pairs with the P1-6 device sweep)                           |
| everything else            | ⬜                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
