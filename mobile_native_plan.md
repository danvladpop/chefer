...
| M2-2  | `meal-plan`                                  | ✅ DONE 2026-08-31: week nav (−1..+1), day chips, meal cards, generate (free/premium + leftovers toggle, PRECONDITION_FAILED upsell), per-slot AI swap, week cost badge. Deferred: photo SSE (M3-1), replace-from-saved picker, swap-context rating on recipe detail, pantry/rebalance banners                                                                                                          | L    |
...
| M2-10 | `household`, `feedback`, `history`, `import` | ✅ PARTIAL 2026-08-31: history (list+restore; [planId] detail deferred) + beta feedback card on More. STILL UNPORTED (explicit deferrals): household management (F2), recipe import (F5), recipe create/edit, cook mode (P1-3), coach review banner + WeightCard/progress charts (need an RN chart lib — pick one first), onboarding wizard. These are the Wave-2 leftovers; schedule as mobile/wave-2b | M    |
