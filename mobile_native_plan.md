...
| ----- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---- |
| M2-1  | `dashboard`                                  | Home tab: today's meals, nutrition summary, quick actions                                                                      | M    |
| M2-2  | `meal-plan`                                  | Plan view, day navigation, swap flow (respect FREE vs PREMIUM gating — see `premiumProcedure` notes in `infrastructure.md` §9) | L    |
| M2-3  | `recipes` + `recipe`                         | List, search/filter, detail (images come from `/uploads/*` — absolute URL via env)                                             | L    |
| M2-4  | `tracker`                                    | Daily log, add/edit entries                                                                                                    | M    |
| M2-5  | `shopping-list`                              | List + check-off                                                                                                               | S    |
| M2-6  | `pantry`                                     | Pantry CRUD                                                                                                                    | S    |
| M2-7  | `preferences` + `onboarding`                 | Onboarding wizard + settings; premium gating on personalisation                                                                | L    |
| M2-8  | `profile` (in `user`/nav)                    | Profile, plan tier display, logout, delete account confirm                                                                     | S    |
