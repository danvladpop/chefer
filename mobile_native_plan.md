...
| ----- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| M2-1  | `dashboard`                                  | ✅ DONE 2026-08-31: week outlook + day panel, nutrition summary (3-state status), hero meal, later-today, favourites, profile nudge. Deviations: calorie ring→bar (reverted 2026-09-26: animated ring, motion P2-1); weight/coach cards deferred to M2-9/M2-10 (need an RN chart lib). Gotcha: testID must sit on Text/Pressable, not a bare View, for Maestro to see it on iOS                                                                                                                                                                                           | M    |
| M2-2  | `meal-plan`                                  | ✅ DONE 2026-08-31: week nav (−1..+1), day chips, meal cards, generate (free/premium + leftovers toggle, PRECONDITION_FAILED upsell), per-slot AI swap, week cost badge, replace-recipe picker sheet (2026-09-23: per-meal button opens a bottom sheet — own/favourite recipes on top, search, pick-a-recipe via `mealPlan.replaceRecipe` on every tier, AI regen footer for premium). Deferred: photo SSE (M3-1), pantry banner. 2026-09-26 (P1-7): rebalance banner + undo on the Plan tab; recipe detail opened from a day (`day`/`meal` params) shows the star rating | L    |
...
| M2-4  | `tracker`                                    | ✅ DONE 2026-08-31: date nav, planned-meal check-off + portion chips, custom-entry rows (helpers moved to @chefer/utils — shared with web), totals vs targets, save. 2026-09-26 (P1-7): QuickAddSheet (meal slot + optional macros) and the rebalance banner with merged undo, shown on the tracker itself. Scan = M3-2                                                                                                                                                                                                                                                   | M    |
...
| M2-10 | `household`, `feedback`, `history`, `import` | ✅ DONE 2026-08-31 (wave-2b complete): history+feedback, then onboarding wizard, cook mode (P1-3, keep-awake + inline timers), recipe import (URL/text; photo waits for M3-2), recipe create/edit forms, household (F2), coach review banner + weight card (View-based sparkline — no chart lib needed). Remaining known gaps: photo import source (history/[planId] detail + /progress charts landed 2026-09-26, P1-7)                                                                                                                                                   | M    |
...
**Progress 2026-09-26 (branch `feat/app-store-readiness`) — App Review blockers, code side:**
...
- Still [USER]: App Privacy questionnaire (declare: contact info, health & fitness / body metrics, photos, user content, usage data (PostHog), diagnostics (Sentry); AI provider = third-party processing), age rating, screenshots, review notes with a demo account.
