...
| 4   | ~~🟠~~ | ~~Recharts `Tooltip` is hover-driven, so tooltips are unreachable on touch.~~ **This was wrong** — measured, a tap fires compatibility mouse events (`mouseover`/`mousemove`/`click`), which is what Recharts listens for, so hover tooltips do open on tap. A `trigger="click"` variant was built, measured against the default, and reverted: both opened on tap and neither dismissed on tapping away. | No trigger change. `activeDot` enlarged to `r: 6`. The non-dismissal is real, affects both triggers, and needs its own fix |
...
- **Touch and hover.** The assumption that hover tooltips are unreachable on touch is false — tapping fires compatibility mouse events and opens them. The genuine problem is that a tooltip opened by tap does not dismiss when you tap away, under either trigger mode. Unresolved.
