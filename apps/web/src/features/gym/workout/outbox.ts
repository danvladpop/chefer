...
// ─── Web outbox (gym_plan.md §5.2, localStorage adapter) ─────────────────────
// A port of apps/mobile/src/features/gym/offline/outbox.ts with the same
// semantics — keep the two in step. Finished and discarded workouts wait here
// until gym.session.upsertMany acks them.
//
...
//  • 'rejected' (or a doc that fails local validation) PARKS the entry
//    ("needs attention" in gym settings: Copy / Retry / Discard);
//  • network / server errors keep everything and back off exponentially;
//  • only the CONFIRMED signed-in owner's entries are ever sent.
...
export function createOutbox(deps: OutboxDeps = {}) {
  ...
  const store = createExternalStore<OutboxState>(() => {
    ...
    // Unreadable queue: keep the bytes for support, start clean.
    storage().setItem(`${GYM_KEYS.outboxQuarantine}.${now()}`, raw);
    return EMPTY_STATE;
  });
  ...
  const outbox = {
     ...
     * running pass. `force` ignores the backoff window (online, focus,
    ...
    /** Re-read from storage (tests; another tab wrote the queue). */
    reload: store.reset,
  };
...
export interface OutboxStatus {
  ...
  parked: OutboxEntry[];
  /** Queued entries that belong to a different account in this browser. */
  otherAccount: number;
  lastSyncAt: string | null;
  lastError: string | null;
  isFlushing: boolean;
...
/** Finished docs waiting for upload for `owner` (the optimistic Today / summary read them). */
export function pendingFinishedDocs(state: OutboxState, owner: string | null): WorkoutSessionDoc[] {
...
const EMPTY_SERVER_STATE = EMPTY_STATE;
const serverState = () => EMPTY_SERVER_STATE;
const serverFalse = () => false;
const serverOwner = () => null;

export function useOutboxState(): OutboxState {
  return useSyncExternalStore(outbox.subscribe, outbox.getState, serverState);
}
...
export function useOutboxStatus(): OutboxStatus {
  ...
  const isFlushing = useSyncExternalStore(outbox.subscribeFlushing, outbox.isFlushing, serverFalse);
