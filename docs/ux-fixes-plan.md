...
| #   | ID   | Item                                                                                                       | Where                                             | Acceptance                                                                      | Status                                                    |
| --- | ---- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------- |
| 5.1 | F-2  | Swap undo toast (5s)                                                                                       | recipe detail / planner swap mutation             | "Swapped — Undo" toast restores prior recipe                                    | ✅ 1419478                                                |
| 5.2 | §5.4 | Photo counter monotonic ("14 of 17")                                                                       | recipe-images SSE + planner chip                  | Denominator fixed for a generation batch; numerator only increases              | ✅ 1419478                                                |
| 5.3 | —    | Profile page: vendor telemetry (Gemini/Pollinations) visible to admins only                                | `/profile` page + user role check                 | Non-admin sees account + plan mgmt only; admin retains telemetry                | ✅ 1419478                                                |
| 5.4 | F-3  | Shopping list: categories collapsed by default with counts + per-category check progress                   | `features/shopping-list/`                         | Collapsed groups w/ "12 items · 3 done"; expand persists per session            | ✅ 1419478                                                |
| 5.5 | F-5  | Consistency sweep: per-page `<title>`s, Pantry header shows "Pantry", orange-title legend on planner cards | layout/header components, planner card            | Every route has title; header matches page; special cards labeled or normalized | ✅ 1419478 (orange titles were hover styling — no defect) |
| 5.6 | M-1  | "SUSTAINABLE CHOICE" badge: tooltip explaining it, or remove                                               | dashboard header                                  | Badge explains itself on hover/tap, or is gone                                  | ✅ 3d93db9                                                |
| 5.7 | M-2  | Chat FAB no longer covers last card's macros on mobile                                                     | planner page bottom padding / FAB scroll behavior | 375px: last card fully readable with FAB present                                | ✅ 1419478                                                |
...
- 2026-08-23 · Phase 5 DONE: 1419478 (swap undo toast + Toast action slot, monotonic photo pill, user-quota profile card w/ vendor telemetry admin-only, collapsible shopping categories, per-page tab titles + /pantry //premium //admin headers, FAB clearance, mobile day-view target badge). The "orange planner titles" from the review were group-hover styling caught mid-screenshot — no defect.
- 2026-08-23 · ALL 24 plan items complete. Verified: 59-test mobile Playwright sweep, api 350 tests, web 34, utils 19, lint 0 errors. NOT pushed — owner pushes to master to deploy.
